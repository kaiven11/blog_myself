...
var echartsAPIList = ['getDom', 'getZr', 'getWidth', 'getHeight', 'getDevicePixelRatio', 'dispatchAction', 'isDisposed', 'on', 'off', 'getDataURL', 'getConnectedDataURL', 'getModel', 'getOption', 'getViewOfComponentModel', 'getViewOfSeriesModel']; // And `getCoordinateSystems` and `getComponentByElement` will be injected in echart.js
...
function getAxisLabelInterval(tickCoords, labels, font, axisRotate, labelRotate) {
  ...
  for (var i = 0; i < tickCoords.length; i += step) {
    ...
    var rect = getBoundingRect(labels[i], font, 'center', 'top');
...
var axisModelCommonMixin = {
  ...
  getFormattedLabels: function () {
    return getFormattedLabels(this.axis, this.get('axisLabel.formatter'));
  },
  ...
  getCategories: function () {
    return this.get('type') === 'category' && map(this.get('data'), getName);
  },
  ...
  getLabelInterval: function () {
    ...
    if (!labelInterval) {
      ...
      if (this.type === 'category' && (labelInterval == null || labelInterval === 'auto')) {
        labelInterval = getAxisLabelInterval(map(this.scale.getTicks(), this.dataToCoord, this), axisModel.getFormattedLabels(), labelModel.getFont(), this.getRotate ? this.getRotate() : this.isHorizontal && !this.isHorizontal() ? 90 : 0, labelModel.get('rotate'));
      }
...
var GaugeView = Chart.extend({
  ...
  _renderMain: function (seriesModel, ecModel, api, colorList, posInfo) {
    ...
    var angleRangeSpan = (endAngle - startAngle) % PI2$4;
  ...
  _renderPointer: function (seriesModel, ecModel, api, getColor, posInfo, startAngle, endAngle, clockwise) {
    ...
    data.eachItemGraphicEl(function (pointer, idx) {
      ...
      setHoverStyle(pointer, itemModel.getModel('itemStyle.emphasis').getItemStyle());
    });
    this._data = data;
  },
...
function collectAxesInfo(result, ecModel, api) {
  ...
  each$9(api.getCoordinateSystems(), function (coordSys) {
    ...
    each$9(coordSys.getAxes(), curry$2(saveTooltipAxisInfo, false, null)); // If axis tooltip used, choose tooltip axis for each coordSys.
    ...
    && baseTooltipModel.get('show')) {
      ...
      if (triggerAxis || cross) {
        each$9(tooltipAxes.baseAxes, curry$2(saveTooltipAxisInfo, cross ? 'cross' : true, triggerAxis));
      }

      if (cross) {
        each$9(tooltipAxes.otherAxes, curry$2(saveTooltipAxisInfo, 'cross', false));
      }
...
function makeAxisPointerModel(axis, baseTooltipModel, globalAxisPointerModel, ecModel, fromTooltip, triggerTooltip) {
  ...
  var volatileOption = {};
  each$9(['type', 'snap', 'lineStyle', 'shadowStyle', 'label', 'animation', 'animationDurationUpdate', 'animationEasingUpdate', 'z'], function (field) {
...
var axisTrigger = function (payload, ecModel, api) {
  ...
  each$10(coordSysAxesInfo.coordSysMap, function (coordSys, coordSysKey) {
    // If a point given, it must be contained by the coordinate system.
    var coordSysContainsPoint = isIllegalPoint || coordSys.containPoint(point);
    each$10(coordSysAxesInfo.coordSysAxesInfo[coordSysKey], function (axisInfo, key) {
      var axis = axisInfo.axis;
  ...
  var linkTriggers = {};
  each$10(axesInfo, function (tarAxisInfo, tarKey) {
    var linkGroup = tarAxisInfo.linkGroup; // If axis has been triggered in the previous stage, it should not be triggered by link.

    if (linkGroup && !showValueMap[tarKey]) {
      each$10(linkGroup.axesInfo, function (srcAxisInfo, srcKey) {
  ...
  each$10(linkTriggers, function (val, tarKey) {
...
function buildPayloadsBySeries(value, axisInfo) {
  ...
  var minDist = Number.MAX_VALUE;
  var minDiff = -1;
  each$10(axisInfo.seriesModels, function (series, idx) {
    ...
    if (dist <= minDist) {
      ...
      each$10(dataIndices, function (dataIndex) {
        payloadBatch.push({
...
function updateModelActually(showValueMap, axesInfo, outputFinder) {
  ...
  each$10(axesInfo, function (axisInfo, key) {
...
function dispatchHighDownActually(axesInfo, dispatchAction, api) {
  ...
  each$10(axesInfo, function (axisInfo, key) {
    var option = axisInfo.axisPointerModel.option;
    option.status === 'show' && each$10(option.seriesDataIndices, function (batchItem) {
...
function initGlobalListeners(zr, api) {
  ...
  function useHandler(eventType, cb) {
    zr.on(eventType, function (e) {
      ...
      each$11(get$3(zr).records, function (record) {
...
function assembleFont(textStyleModel) {
  ...
  each$13(['decoration', 'align'], function (name) {
...
function assembleCssText(tooltipModel) {
  ...
  each$13(['width', 'color', 'radius'], function (name) {
...
extendComponentView({
  ...
  _showAxisTooltip: function (dataByCoordSys, e) {
    ...
    each$12(dataByCoordSys, function (itemCoordSys) {
      // var coordParamList = [];
      ...
      each$12(itemCoordSys.dataByAxis, function (item) {
  ...
  _updateContentNotChangedOnAxis: function (dataByCoordSys) {
    ...
    contentNotChanged && each$12(lastCoordSys, function (lastItemCoordSys, indexCoordSys) {
      var lastDataByAxis = lastItemCoordSys.dataByAxis || {};
      var thisItemCoordSys = dataByCoordSys[indexCoordSys] || {};
      var thisDataByAxis = thisItemCoordSys.dataByAxis || [];
      contentNotChanged &= lastDataByAxis.length === thisDataByAxis.length;
      contentNotChanged && each$12(lastDataByAxis, function (lastItem, indexAxis) {
        var thisItem = thisDataByAxis[indexAxis] || {};
        var lastIndices = lastItem.seriesDataIndices || [];
        var newIndices = thisItem.seriesDataIndices || [];
        contentNotChanged &= lastItem.value === thisItem.value && lastItem.axisType === thisItem.axisType && lastItem.axisId === thisItem.axisId && lastIndices.length === newIndices.length;
        contentNotChanged && each$12(lastIndices, function (lastIdxItem, j) {
